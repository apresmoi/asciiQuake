 ...
 *
 * Tone defaults (2026-09 retune vs cssquake.wtf, unobstructed e1m1 gameplay):
 * the earlier reference capture had cssQuake's 65%-black menu veil open, so
 * it was only 35% of the real gameplay target. `brighten` now reaches sampled
 * wall/floor texture ink as well as flat geometry; it does not change glyph
 * selection, Quake's baked lightmaps, or animated lightstyles. The final
 * defaults were tuned against unobstructed cssQuake gameplay. */
export const QUAKE_GLYPH_WORLD_TUNING_KNOBS: readonly QuakeGlyphTuningKnob[] = [
  { key: "brighten", param: "glyphBright", label: "brighten (linear)", min: 1, max: 12, step: 0.1, def: 3.5, group: "World tone" },
  ...
  { key: "flat", param: "glyphFlat", label: "flatten colours", min: 0, max: 1, step: 0.02, def: 0.5, group: "World tone" },
  { key: "stroke", param: "glyphStroke", label: "glyph stroke px", min: 0, max: 2, step: 0.05, def: 0.4, group: "World tone" },
  { key: "ambient", param: "glyphAmbient", label: "ambient light", min: 0, max: 1, step: 0.02, def: 0.55, group: "World light" },
  { key: "dir", param: "glyphDir", label: "directional light", min: 0, max: 1, step: 0.02, def: 0.5, group: "World light" },
